...
from elftools.dwarf.descriptions import (
    describe_attr_value_extra, describe_reg_name, describe_attr_value, set_global_machine_arch,
...
class ReadElf(object):
        ...
        for cu in self._dwarfinfo.iter_CUs():
            ...
            for die in cu.iter_DIEs():
                ...
                for attr in die.attributes.values():
                    ...
                    if(current_function!=die and current_function!=None):
                        print("func where")
    ...
    class FuncManager:
        ...
        def __init__(self, _debugInfo_offset):
            ...
            self.cfa_pcs = []
            self.cfas = [] # CFARule, defined in callframe.py
            self.func = None    # func die
            
            # only consider DW_OP_fbreg now
            self.vars_list = []
            self.var_entries = []   # DIEs
        
        def getVar(self, pc, reg: str, offset):
            pc_index = bisect.bisect_right(self.cfa_pcs, pc) - 1
            if(pc_index < 0):
                print("wrong in find last cfa row")
                return
            cfa = self.cfas[pc_index]
            ...
            offset = int(offset) - cfa_offset    # new offset is the used addr - base
            var_index = bisect.bisect_right(self.vars_list, offset) - 1
            if(var_index<0):
                return
    ...
    def sort_entries_withId(self, entries: list,  findId):
        ...
        return list(map(findId, entries))

    def id_FDERow(self, row:dict):
        return row["pc"]

    def id_VarDIE(self, die:DIE):
        # sort by addr of var
        desc = describe_attr_value_extra(die.attributes['DW_AT_location'], die, self._dwarfinfo.debug_info_sec.global_offset)
        return int(re.findall(r"\-?\d+", desc)[0])

    def id_funMgr(self, func:FuncManager):
        # sort by low pc of func
        # attr = func.func.attributes
        # if 'DW_AT_low_pc' in attr:
        #     return attr['DW_AT_low_pc'].value
        # elif 'DW_AT_entry_pc' in attr:
        #     return attr['DW_AT_entry_pc'].value
        # else:
        #     print("no low pc, must be only one function")
        #     return 0
        return func.low_pc
    ...
    def show_locals(self):
        ...
        for cu in self._dwarfinfo.iter_CUs():
            ...
            for die in cu.iter_DIEs():
                ...
                if die.tag == "DW_TAG_variable" and self.check_var(die, debugInfo_offset):
                    # print(f' send {die.attributes["DW_AT_name"]}')
                    var_entries.append(die)
                    if(current_function):
                        var_funcMap[die] = current_function
                        current_mgr.var_entries.append(die)
        
        
                

        # sort vars in each funMgr
        for mgr in self.FuncManager.func_entries:
            mgr.vars_list = self.sort_entries_withId(mgr.var_entries, self.id_VarDIE)
        # sort funcs
        self.FuncManager.func_list = self.sort_entries_withId(self.FuncManager.func_entries, self.id_funMgr)
        

        frame_entries = []
        if self._dwarfinfo.has_EH_CFI():
            frame_entries = self._dwarfinfo.EH_CFI_entries()
        if self._dwarfinfo.has_CFI():
